...
def main():
    ...
    for ev in evs:
        ...
        bips=base.num(ev.get('negRiskFeeBips'));recs.append({'event_id':ev.get('id'),'title':ev.get('title'),'slug':ev.get('slug'),'endDate':ev.get('endDate'),'negRiskFeeBips':bips,'conversion_fee_fraction':bips/10000.0,'markets':ms})
    ...
    for ev in recs:
        ...
        for i,maker in enumerate(legs):
            ...
            for q0 in Q_GRID:
                ...
                setups.append({'event_id':ev['event_id'],'event':ev['title'],'slug':ev['slug'],'endDate':ev['endDate'],'n_legs':nlegs,'negRiskFeeBips':ev['negRiskFeeBips'],
    ...
    out={'generated_at':time.time(),'method':{'q_grid':list(Q_GRID),'reserve_usd':RESERVE_USD,'target_profit_usd':TARGET_PROFIT_USD,'min_conditional_roi':MIN_CONDITIONAL_ROI,'maker_fee':0,'maker_rebate_credited':False,'adapter_fee_included':True,
